...
class OmniglotDataset(data.Dataset):
    ...
    def __init__(self, mode='train', root='c:'+os.sep+'Users'+os.sep+'User'+os.sep+'Desktop'+os.sep+'research_code'+os.sep+'data'+os.sep+'original'+os.sep+'omniglot_pre', transform=None, target_transform=None, download=True):
...
class OmniglotFeatures(data.Dataset):
    def __init__(self, split='train', path="./data/preomniglot"):

        datafile = torch.load(os.path.join(path, split + "_file.pt"))
    ...
    def __getitem__(self, i):
        img, lbl = self.data[i], self.label[i]
        return img, lbl

if __name__ == '__main__':
    o = OmniglotDataset(mode='train')
    print(o.__len__())