...
from convnet import DistillKL, Convnet1d1c
...
def main(args):
    ...
    teacher = Convnet1d1c().cuda()
    teacher.load_state_dict(torch.load(os.path.join(args.pretrain_path, 'max-acc.pth')))
...
def train(args, teacher, model, train_loader, optimizer, kd_loss):
    model.train()
    ...
    for i, batch in enumerate(train_loader, 1):
        ...
        proto = None; logits = None; loss = None; tproto = None; tlogits = None
...
if __name__ == '__main__':
    ...
    parser.add_argument('--pretrain-path', default='./save/omniglot-t')
    parser.add_argument('--save-path', default='./save/omniglot-s')
